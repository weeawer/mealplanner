...
namespace api.Controllers
{
    [Route("api/choise")]
    [ApiController]
    public class ChoiseController(IMealRepository mealRepository, IChoiseMealsSwapRepository choiseMealsSwapRepository, IDayRepository dayRepository, IAppUserRepository appUserRepository) : ControllerBase
    {
        private readonly IChoiseMealsSwapRepository _choiseMealsSwapRepository = choiseMealsSwapRepository;
        ...
        public async Task<IActionResult> AddMeals(List<DayMealsDto> dayMealsDtos)
        {
            foreach (var dayMealsDto in dayMealsDtos)
            {
                ...
                foreach (var item in dayMealsDto.MealsId)
                {
                    ...
                    {
                        var choiceMeal = new ChoiseMealsSwap()
                        {
                            AppUserId = user.Id,
                            DayId = day.Id,
                            MealId = item,
                        };
                        await _choiseMealsSwapRepository.AddAsync(choiceMeal);
                    }
                }
            }
